...
def load_messages_winners(filename="winners.txt"):  # Corrigir o nome do arquivo
    with open(filename, 'r', encoding='utf-8') as file:
        content = file.read()
    ...
    for message in messages:
        ...
        if nome_match and link_match:
            winners.append({
                ...
                'nome': nome_match.group(1).strip(),
                'votos': int(votos_match.group(1)) if votos_match else 0,
                'link': link_match.group(1).strip(),
                'descricao': desc_match.group(1).strip() if desc_match else '',
                'usuario': user_match.group(1).strip() if user_match else '',
                'votar': message_link_match.group(1).strip() if message_link_match else '',
    ...
    return sorted(winners, key=lambda x: x['rank'])  # Ordena por rank
...
def main():
    ...
    st_ranking, st_vencedores = st.tabs(["Ranking", "Vencedores"])
            ...
            with col:
                ...
                if medal_class:
                    st.markdown(
                        f"""
                        <div class="project-name-container">
                            <h5 class="project-name">{winner['rank']}. <span class="{medal_class}">{winner['nome']}</span></h5>
                        </div>
                        """,
                        unsafe_allow_html=True
                    )
                    ...
                    st.markdown(
                        f"""
                        <div class="project-name-container">
                            <h5 class="project-name">{winner['rank']}. {winner['nome']}</h5>
                        </div>
                        """,
                        unsafe_allow_html=True
                    )
                ...
                col1, col2 = st.columns(2)
                with col1:
                    st.link_button("Github", url=winner['link'], type="primary", use_container_width=True)
                with col2:
                    if st.button("Detalhes", key=f"detalhes_winner_{idx}", use_container_width=True):
                        st.session_state.selected_project = winner
                        show_details()
